...
using bgpd.Readers;
using static bgpd.Readers.CREReader;
    ...
    public class BGEntity
    {
        ...
        public List<string> Protections
        {
            get
            {
                ...
                if (DerivedStats != null && DerivedStats.WeaponImmune.Count > 0)
                {
                ...
                for (int i = 9; i > 0; --i)
                {
                    if (DerivedStatsTemp != null && DerivedStatsTemp.spellImmuneLevel[i] > 0)
                    {
        ...
        private void Init(ResourceManager resourceManager, IntPtr entityIdPtr)
        {
            ...
            try
            {
                ...
                this.Id = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x48]));
                this.Type = WinAPIBindings.ReadByte(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x8]));

                if (Type != 49)
                    return;

                this.X = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(entityIdPtr, [0xC]));
                this.Y = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(entityIdPtr, [0xC + 4]));

                if (X < 0 || Y < 0)
                    return;

                this.CreResourceFilename = WinAPIBindings.ReadString(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x540]), 8).Trim('*') + ".CRE";

                IntPtr cGameAreaPtr = WinAPIBindings.FindDMAAddy(entityIdPtr, [0x18]);
                this.EnemyAlly = WinAPIBindings.ReadByte(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x38]));
                this.RACE = (RACE)WinAPIBindings.ReadByte(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x3A]));
                this.CLASS = (CLASS)WinAPIBindings.ReadByte(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x3B]));

                if (this.CreResourceFilename == ".CRE")
                    return;
                this.cInfGamePtr = WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x228]);
                this.UpdateTime();
                this.AreaName = WinAPIBindings.ReadString(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x0]), 8);
                this.MousePosX = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x254]));
                this.MousePosY = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x254 + 4]));
                this.CInfinityPtr = cGameAreaPtr + 0x5C8;
                this.MousePosX1 = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x5C8 + 0x60]));
                this.MousePosY1 = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x5C8 + 0x60 + 0x4]));
                this.ViewportHeight = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x5C8 + 0x78 + 0xC]));
                this.ViewportWidth = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(cGameAreaPtr, [0x5C8 + 0x78 + 0x8]));
                this.Name2 = WinAPIBindings.ReadString(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x3928, 0x0]), 64);
                this.Name1 = WinAPIBindings.ReadString(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x30, 0x0]), 8);
                this.CurrentHP = WinAPIBindings.ReadInt16(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x560 + 0x1C]));
                this.timedEffectsPointer = WinAPIBindings.FindDMAAddy(entityIdPtr, [0x4A00]);
                this.equipedEffectsPointer = WinAPIBindings.FindDMAAddy(entityIdPtr, [0x49B0]);
                this.curSpellPtr = entityIdPtr + 0x4AE0; //TODO: Current spell being cast? should be pretty cool
                this.equipmentPtr = WinAPIBindings.FindDMAAddy(entityIdPtr, [0xFC0]);
                this.isInvisible = WinAPIBindings.ReadInt32(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x4928]));
                this.Loaded = true;
        ...
        {
            this.GameTime = WinAPIBindings.ReadUInt32(WinAPIBindings.FindDMAAddy(cInfGamePtr, [0x3FA0]));
        }

        public void LoadDerivedStats()
        {
            this.DerivedStats = new CDerivedStats(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x1120]));
            this.DerivedStatsBonus = new CDerivedStats(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x2A70]));
            this.DerivedStatsTemp = new CDerivedStats(WinAPIBindings.FindDMAAddy(entityIdPtr, [0x1DC8]));
        ...
        public static Dictionary<ushort, string> EnemyAllyDict = new()
        {
            { 0, "Anyone" },
            { 1, "Inanimate" },
